...
find_twitter_handle_from_bing <- function(org_name,
    ...
    resp <- GET(url = glue("{base_url}/?q={search_term}"), 
...
parse_twitter_handle_from_page <- function(website_address, google_search_results = NULL) {
    ...
    all_links <- data.frame(href = tolower(hrefs), # lower cases  
                            link_text = tolower(link_texts)) # lower cases
...
find_all <- function(org_name, website_address = NULL) {
    ...
    # Combined output 
    if (is.null(tw_org) == TRUE) {out <- tw_bing} 
